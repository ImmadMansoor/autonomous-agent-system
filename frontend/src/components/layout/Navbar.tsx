...
export function Navbar({ 
  ...
  useEffect(() => {
    ...
    // Listen for live signals to update notifications in real-time
    ...
    import('socket.io-client').then(({ io }) => {
      ...
      socket.on('signal:created', (signal: any) => {
        setNotifications(prev => {
          // Avoid duplicate notifications
          if (prev.some(n => n.id === signal.id)) return prev;
          
          const newNotif = {
            id: signal.id,
            message: signal.message,
            time: new Date(signal.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
            read: false,
          };
