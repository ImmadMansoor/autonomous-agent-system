...
export default function Navbar() {
  ...
  return (
    <div className="w-full flex justify-center pt-4 sm:pt-6 px-3 sm:px-4 shrink-0">
      <nav className="bg-white/90 backdrop-blur-md rounded-full shadow-sm border border-zinc-300/60 pl-3 pr-3 py-2 w-full max-w-[840px] relative flex items-center">
        {/* Logo */}
        ...
        <div className="ml-auto flex items-center gap-2">
          {/* Activity Status */}
          <div className="hidden sm:flex items-center gap-1.5 text-xs text-[#00685f] px-2.5 py-1 bg-[#e0f1ee] rounded-full font-medium">
          ...
          <button 
            ...
            className="bg-[#00685f] text-white rounded-full pl-4 pr-1.5 py-1.5 text-[13px] sm:text-[14px] font-medium flex items-center gap-2 hover:bg-[#00524a] transition-all cursor-pointer select-none"
          >
            <span>Console</span>
        ...
        {isOpen && (
          <div className="absolute top-full left-2 right-2 mt-2 bg-white rounded-2xl shadow-lg border border-zinc-300/60 p-4 z-20 md:hidden flex flex-col gap-3">
            {navItems.map((item) => {
              const isTelemetry = item.id === "telemetry";
              return (
                <button
